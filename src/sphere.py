...
def plot_edge_densities_sphere(N, A, output_dir=""):
    ...
    edge_density_histogram(degrees, max_degree(nodes), graph_topology="sphere", name="%s%d_%d.png" % (output_dir, N, A))
...
def plot_runtime_chart(Ns, A, output_dir=""):
    ...
    runtime_chart(Ns, runtimes, graph_topology="Sphere", name="%sruntime_chart.png" % output_dir, expected_order=1)
    print("\nDone Creating Runtime Chart")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate the charts for sphere sensor network topology")
    ...
    if args.edge_density:
        ...
        N = 8000
        plot_edge_densities_sphere(N, A, output_dir="../results/sphere/edge_density/")

    if args.runtimes:
        Ns = [1000, 4000, 8000, 16000, 32000]
        plot_runtime_chart(Ns, A,  output_dir="../results/sphere/runtime/")
    ...
    if args.test:
        ...
        print("R:                       %f" % calculate_radius_sphere(N, A))
    ...
    if args.draw:
        ...
        N=1000
        nodes = unit_sphere_graph(N, A)
        draw_nodes(nodes, num_nodes=N, topology="sphere",output_dir="../results/sphere/drawing/")
